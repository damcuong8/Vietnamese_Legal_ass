...
def make_nvidia_get_llm(
    ...
    def get_nvidia_llm(
        ...
        if not api_key:
            raise RuntimeError("Missing NVIDIA_API_KEY. Export it before using --llm-backend nvidia.")
...
def run_one_item(app, item: dict[str, Any], prefer_applied: bool) -> dict[str, Any]:
    ...
    state = app.invoke({"question": item["question"]}, config)
...
def main() -> None:
    ...
    if args.llm_backend == "nvidia":
        ...
        print(f"[LLM] Using NVIDIA endpoint model={args.nvidia_model}")
    else:
    ...
    def handle_completed_future(future, index: int, item: dict[str, Any]) -> None:
        ...
        try:
            ...
            diagnostics["num_relevant_docs"] = len(relevant_docs)
            diagnostics["num_relevant_articles"] = len(relevant_articles)
